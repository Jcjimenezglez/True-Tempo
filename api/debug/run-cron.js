        ...
        if (scheduledEmails.checkoutAbandoned1 && !scheduledEmails.checkoutAbandoned1Sent && now >= scheduledEmails.checkoutAbandoned1) {
          ...
          if (result.success) {
            await clerk.users.updateUser(user.id, {
              publicMetadata: {
                ...metadata,
                scheduledEmails: { ...scheduledEmails, checkoutAbandoned1Sent: true },
              },
            });
            userDetails.emailsSent.push('checkoutAbandoned1');
            results.emailsSent++;
          } else {
            results.errors.push({ user: userEmail, email: 'checkoutAbandoned1', error: result.error });
          }
        }
        
        // Check checkout abandoned 2
        if (scheduledEmails.checkoutAbandoned2 && !scheduledEmails.checkoutAbandoned2Sent && now >= scheduledEmails.checkoutAbandoned2) {
          const template = templates.getCheckoutAbandonedEmail2({ firstName });
          const result = await sendEmail({
            to: userEmail,
            subject: template.subject,
            html: template.html,
          ...
          if (result.success) {
            await clerk.users.updateUser(user.id, {
              publicMetadata: {
                ...metadata,
                scheduledEmails: { ...scheduledEmails, checkoutAbandoned2Sent: true },
              },
            });
            userDetails.emailsSent.push('checkoutAbandoned2');
            results.emailsSent++;
          } else {
            results.errors.push({ user: userEmail, email: 'checkoutAbandoned2', error: result.error });
          }
        }
        
        // Check checkout abandoned 3
        if (scheduledEmails.checkoutAbandoned3 && !scheduledEmails.checkoutAbandoned3Sent && now >= scheduledEmails.checkoutAbandoned3) {
          const template = templates.getCheckoutAbandonedEmail3({ firstName });
          const result = await sendEmail({
            to: userEmail,
            subject: template.subject,
            html: template.html,
          ...
          if (result.success) {
            await clerk.users.updateUser(user.id, {
              publicMetadata: {
                ...metadata,
                scheduledEmails: { ...scheduledEmails, checkoutAbandoned3Sent: true },
              },
            });
            userDetails.emailsSent.push('checkoutAbandoned3');
            results.emailsSent++;
          } else {
            results.errors.push({ user: userEmail, email: 'checkoutAbandoned3', error: result.error });
          }
        }
