...
/**
 * Script to update Stripe Premium price from 30-day trial to 7-day trial
 * 
 * This script will:
 * 1. Find the current Premium price with 30-day trial
 * 2. Create a new price with 7-day trial
 * 3. Archive the old price
 * 4. Output the new STRIPE_PRICE_ID_PREMIUM to update in Vercel
 * 
 * Usage:
 *   STRIPE_SECRET_KEY=sk_live_xxx node scripts/update-trial-to-7-days.js
 */
...
async function main() {
  ...
  if (!secretKey) {
    ...
    console.log('\nUsage:');
    console.log('  STRIPE_SECRET_KEY=sk_live_xxx node scripts/update-trial-to-7-days.js');
    process.exit(1);
  }
  ...
  const currentPrice = prices.data.find(p => 
    ...
    p.recurring.trial_period_days >= 14  // Current is likely 30 days
  );
  ...
  if (!currentPrice) {
    ...
    console.error('\n❌ Could not find price with trial period >= 14 days');
    process.exit(1);
  }
  ...
  // Create new price with 7-day trial
  console.log('🆕 Creating new price with 7-day trial...\n');
  ...
  const newPrice = await stripe.prices.create({
    ...
    metadata: {
      plan_type: 'premium',
      trial_days: '7',
      created_by: 'update-trial-to-7-days.js',
  ...
  console.log('═══════════════════════════════════════════════════════════════');
  console.log('');
  console.log('🎉 SUCCESS! New 7-day trial price created.');
  console.log('');
  ...
  console.log('   git add .');
  console.log('   git commit -m "Update trial period from 30 days to 7 days"');
