...
// ==========================================
// Exit Intent Modal (Free Users Only)
// ==========================================

(function() {
    let exitIntentShown = false;
    let minTimeOnPage = 60000; // 60 seconds
    let pageLoadTime = Date.now();
    
    // Check if user is authenticated but not Premium (Free user)
    function isFreeUser() {
        if (!window.pomodoroTimer) return false;
        
        const isAuth = window.pomodoroTimer.isAuthenticated;
        const isPremium = typeof window.pomodoroTimer.isPremiumUser === 'function' 
            ? window.pomodoroTimer.isPremiumUser() 
            : false;
        
        return isAuth && !isPremium;
    }
    
    // Check if user has visited pricing page in this session
    function hasVisitedPricing() {
        return sessionStorage.getItem('visitedPricing') === 'true';
    }
    
    // Show exit intent modal
    function showExitIntentModal() {
        const overlay = document.getElementById('exitIntentModalOverlay');
        if (!overlay) return;
        ...
        // Track event
        if (window.pomodoroTimer && typeof window.pomodoroTimer.trackEvent === 'function') {
            window.pomodoroTimer.trackEvent('Exit Intent Modal Shown', {
                user_type: 'free_user',
    ...
    function handleMouseOut(e) {
        ...
        if (e.clientY < 0) {
            ...
            const hasBeenOnPageLongEnough = timeElapsed >= minTimeOnPage;
            const alreadyShown = exitIntentShown || sessionStorage.getItem('exitIntentShown') === 'true';
            const visitedPricing = hasVisitedPricing();
            const userIsFree = isFreeUser();
            
            if (userIsFree && !alreadyShown && !visitedPricing && hasBeenOnPageLongEnough) {
                showExitIntentModal();
            }
        }
    }
    ...
    function initExitIntent() {
        ...
        setTimeout(() => {
            ...
            const closeBtn = document.getElementById('closeExitIntentModalX');
            if (closeBtn) {
                closeBtn.addEventListener('click', function() {
                    hideExitIntentModal();
                    
                    if (window.pomodoroTimer && typeof window.pomodoroTimer.trackEvent === 'function') {
                        window.pomodoroTimer.trackEvent('Exit Intent Modal Dismissed', {
                            user_type: 'free_user'
                        });
                    }
                });
            ...
            const notNowBtn = document.getElementById('exitIntentNotNowBtn');
            if (notNowBtn) {
                notNowBtn.addEventListener('click', function() {
                    hideExitIntentModal();
                    
                    if (window.pomodoroTimer && typeof window.pomodoroTimer.trackEvent === 'function') {
                        window.pomodoroTimer.trackEvent('Exit Intent Not Now Clicked', {
                            user_type: 'free_user'
                        });
                    }
                });
            ...
            // "Start FREE Trial" button handler
            const startTrialBtn = document.getElementById('exitIntentStartTrialBtn');
            if (startTrialBtn) {
                startTrialBtn.addEventListener('click', function() {
                    ...
                    // Mark pricing as visited so modal doesn't show again
                    sessionStorage.setItem('visitedPricing', 'true');
            ...
            // Track if user visits pricing page (to prevent showing modal again)
            if (window.location.pathname.includes('/pricing')) {
                sessionStorage.setItem('visitedPricing', 'true');
            }
        }, 1500); // Wait 1.5s for pomodoroTimer to initialize
    }
