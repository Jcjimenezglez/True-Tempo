class PomodoroTimer {
    ...
    // Time-based escalation: show motivational modal after user has been around for a while
    checkTimeBasedEscalation() {
        try {
            // Only for authenticated, non‑Pro users
            if (!this.isAuthenticated || !this.user) return;
            if (this.isPremiumUser()) return;
            
            const STORAGE_KEY = 'time_based_escalation_shown';
            try {
            ...
            // Determine signup date from Clerk user
            let createdAt = this.user.createdAt || this.user.created_at || this.user.created_at_ms;
            ...
            if (!createdAtMs) {
                return; // Can't safely compute days since signup
            }
            
            const now = Date.now();
            const daysSinceSignup = Math.floor((now - createdAtMs) / (24 * 60 * 60 * 1000));
            
            // Only show after at least 7 full days
            if (daysSinceSignup < 7) return;
            
            // Get total focus hours from existing stats helper (if available)
            let totalHours = 0;
            try {
                const stats = this.getFocusStats ? this.getFocusStats() : null;
                if (stats && typeof stats.totalHours === 'number') {
                    totalHours = stats.totalHours;
                }
            } catch (_) {}
            
            this.showTimeBasedEscalationModal({
                daysSinceSignup,
                totalHours
            });
            ...
            console.error('Error in checkTimeBasedEscalation:', error);
        }
    }
    
    showTimeBasedEscalationModal({ daysSinceSignup = 7, totalHours = 0 } = {}) {
        try {
            // Round total hours to one decimal for display
            const roundedHours = Number.isFinite(totalHours)
            ...
            modal.innerHTML = `
                <button class="close-logout-modal-x" aria-label="Close">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                ...
                <div class="upgrade-content">
                    <div class="upgrade-icon">
                        <svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-trending-up-icon lucide-trending-up">
                            <path d="M16 7h6v6"></path>
                            <path d="m22 7-8.5 8.5-5-5L2 17"></path>
                        </svg>
                    </div>
                    <h3>Your focus habit is growing</h3>
                    <p>You're building a powerful focus habit! You've focused for <strong>${roundedHours.toFixed(1)} hours</strong> total. Every session is getting you closer to your goals. Keep the momentum going with unlimited focus time.</p>
                    <div class="logout-modal-buttons">
                        <button class="logout-modal-btn logout-modal-btn-primary" id="timeEscalationUpgradeBtn">Upgrade to Premium - 3 months free</button>
                        <button class="logout-modal-btn logout-modal-btn-secondary" id="timeEscalationMaybeLaterBtn">Maybe later</button>
                    </div>
                </div>
            `;
            ...
            const closeBtn = modal.querySelector('.close-logout-modal-x');
            if (closeBtn) {
            ...
            const maybeLaterBtn = modal.querySelector('#timeEscalationMaybeLaterBtn');
            if (maybeLaterBtn) {
            ...
            const upgradeBtn = modal.querySelector('#timeEscalationUpgradeBtn');
            if (upgradeBtn) {
                upgradeBtn.addEventListener('click', (e) => {
                    e.preventDefault();
                    
                    // Track upgrade intent from time-based escalation modal
                    this.trackEvent('Subscribe Clicked', {
                        source: 'time_based_escalation_modal',
                        days_since_signup: daysSinceSignup,
                        total_focus_hours: roundedHours
                    });
                    
                    closeModal();
                    
                    // Reuse existing Subscribe button flow if available
                    const subscribeBtn = document.getElementById('subscribeBtn');
                    if (subscribeBtn) {
                        subscribeBtn.click();
                    } else {
            ...
            this.trackEvent('Time-Based Escalation Modal Shown', {
                days_since_signup: daysSinceSignup,
                total_focus_hours: roundedHours
            });
        } catch (error) {
            console.error('Error showing time-based escalation modal:', error);
        }
    }
